...
class SchoolMetrics(DBContext):
    ...
    async def _get_applicants_count(self) -> int:
    ...
    async def _get_students_count(self) -> int:
        async with self.session() as session:
            stmt = (
                select(func.count())
                .select_from(Applicant)
    ...
    async def _get_avg_gpa(self) -> float:
        async with self.session() as session:
            stmt = (
    ...
    async def _get_avg_score(self) -> float:
        async with self.session() as session:
            stmt = (
    ...
    async def _get_top_universities(
            self,
            top_n: int = 3
    ) -> List[Tuple[str, int]]:
        async with self.session() as session:
    ...
    async def _get_olympiads_count(self) -> int:
        async with self.session() as session:
            stmt = (
                select(func.count())
                .select_from(Applicant)
    ...
    async def _get_genders_count(self) -> List[Tuple[str, int]]:
        async with self.session() as session:
            stmt = (
    ...
    async def get_metrics(self) -> Tuple[Any]:
        tasks: List[Coroutine] = [
            self._get_applicants_count(),
            self._get_students_count(),
            self._get_genders_count(),
            self._get_olympiads_count(),
            self._get_avg_gpa(),
            self._get_avg_score(),
            self._get_top_universities(),
            self._get_top_directions()
        ]
